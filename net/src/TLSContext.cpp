...
TLSContext::TLSContext(const TLSConfig& config) : ctx_(nullptr), config_(config) {}
...
bool TLSContext::initialize() {
    ...
    if (!ctx_) {
        handleSslError("Failed to create TLS context");
        return false;
    }
...
bool TLSContext::loadCertificates() {
    // 加载证书
...
void TLSContext::setupSessionCache() {
...
void TLSContext::handleSslError(const char* msg) {
    char buf[256];
